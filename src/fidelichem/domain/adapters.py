...
class MDRunRecord(DomainModel):
    """Canonical representation of a molecular dynamics simulation run."""

    run_name: str
    target_name: str | None = None
    compound_source_value: str | None = None
    source_pose_id: str | None = None
...
class MDMetricRecord(DomainModel):
    ...
    source_pose_id: str | None = None
    source_artifact_path: SafeRelativePath | None = None
    time_points: tuple[float, ...] = Field(default_factory=tuple)
...
class ImportBundle(DomainModel):
    ...
    md_runs: tuple[MDRunRecord, ...] = Field(default_factory=tuple)
    md_metrics: tuple[MDMetricRecord, ...] = Field(default_factory=tuple)
